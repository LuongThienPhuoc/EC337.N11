...
const Payment = () => {
    ...
    useEffect(() => {
        ...
        socketRef.current.on('getListCart', (data) => {
            setUserSelect(data.users.find(cart => cart.userIdSocket === data.idSocket))
            setUser(data.users)
        })
        return () => {
    ...
    const updateCart = (idProduct, amount) => {
        socketRef.current.emit("update-cart", { idSocket: userSelect.userIdSocket, idProduct, amount })
        let newList = [...users]
        ...
        setUser([...newList])
        const newUserSelect = newList.find(user => user.userIdSocket === userSelect.userIdSocket)
                    ...
                    <Row style={{ marginTop: "50px" }} gutter={[16, 16]}>
                        ...
                        <Col span={10} offset={3}>
                            <div style={{ display: "flex", alignItems: "center" }}>
                                <h1 style={{ marginBottom: "30px", paddingRight: "20px" }}>Users:</h1>
                                <Avatar.Group maxCount={2} maxStyle={{ color: '#f56a00', backgroundColor: '#fde3cf', marginBottom: "50px" }}>
                                    {
                                        users.map(user => {
                                            return (
                                                <Tooltip onClick={() => { setUserSelect(user) }} title={user.userIdSocket} placement="top">
                                                    <Avatar size={50} style={{ backgroundColor: '#f56a00', cursor: "pointer" }}>{user.userIdSocket}</Avatar>
                                                </Tooltip>
                                            )
                                        })
                                    }
